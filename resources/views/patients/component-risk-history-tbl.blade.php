<h1>3. ประวัติเสี่ยง</h1>
<div class="form-row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 mb-3">
		<table class="table">
			<thead class="bg-warning">
				<tr>
					<th scope="col">รายการ</th>
					<th scope="col">#</th>
					<th scope="col">#</th>
				</tr>
			</thead>
			<tfoot>
				<tr><td colspan="3">&nbsp;</td></tr>
			</tfoot>
			<tbody>
				<tr>
					<td>ช่วง 7 วันก่อนป่วยได้สัมผัสสัตว์ปีกป่วย/ตายโดยตรง</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>
						<div class="form-group row mt-0 mb-0">
							<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 col-xl-10">
								<div class="input-group">
									<label for="" class="font-normal">ช่วง 14 วันก่อนป่วยได้สัมผัสสัตว์ป่วยโดยตรงหรือไม่ ระบุชนิดสัตว์</label>
									<input type="text" name="" class="form-control form-control-sm ml-2">
								</div>
							</div>
						</div>
					</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>ช่วง 14 วันก่อนป่วยได้พักอาศัยอยู่ในพื้นที่ที่มีสัตว์ปีกป่วย/ตายผิดปกติ</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>
						<div class="form-group row mt-0 mb-0">
							<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 col-xl-10">
								<div class="input-group">
									<label for="" class="font-normal">ช่วง 14 วันก่อนป่วยได้พักอาศัยอยู่หรือเดินทางมาจากพื้นที่ที่ไข้หวัดใหญ่/ปอดอักเสบระบาด <span class="text-info">ระบุพื้นที่</span></label>
									<input type="text" name="" class="form-control form-control-sm ml-2">
								</div>
							</div>
						</div>
					</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>ช่วง 14 วันก่อนป่วยได้ดูแลหรือสัมผัสใกล้ชิดกับผู้ป่วยอาการคล้ายไข้หวัดใหญ่หรือปอดอักเสบ</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>ช่วง 14 วันก่อนป่วยไปเยี่ยมผู้ป่วยไข้หวัดใหญ่หรือปอดอักเสบ</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>เป็นบุคลากรทางการแพทย์และสาธารณสุขหรือเจ้าหน้าที่ห้องปฏิบัติการ</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>เป็นผู้ป่วยสงสัยไข้หวัดใหญ่/ปอดอักเสบที่เข้ารับการรักษาเป็นกลุ่มก้อน</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
				<tr>
					<td>
						<div class="form-group row mt-0 mb-0">
							<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 col-xl-10">
								<div class="input-group">
									<label for="" class="font-normal">อื่นๆ ระบุ</label>
									<input type="text" name="" class="form-control form-control-sm ml-2" disabled style="width:400px;">
								</div>
							</div>
						</div>
					</td>
					<td class="text-danger">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ไม่ใช่</label>
						</div>
					</td>
					<td class="text-success">
						<div class="custom-control custom-checkbox">
							<input type="checkbox" value="N" class="custom-control-input">
							<label for="" class="custom-control-label normal-label">ใช่</label>
						</div>
					</td>
				</tr>
			</tbody>
		</table>
	</div>
</div>



<div class="row">
	<div class="form-row">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 mb-3">
			<label for="">ผลการรักษา</label>
			<div>
				<div class="custom-control custom-checkbox custom-control-inline">
					<input type="checkbox" name="patientCheckbox" class="custom-control-input" id="opdCheckbox">
					<label for="opdCheckbox" class="custom-control-label normal-label">หาย</label>
				</div>
				<div class="custom-control custom-checkbox custom-control-inline">
					<input type="checkbox" name="patientCheckbox" class="custom-control-input" id="ipdCheckbox">
					<label for="ipdCheckbox" class="custom-control-label normal-label">อยู่ระหว่างการรักษา</label>
				</div>
				<div class="custom-control custom-checkbox custom-control-inline" style="width:340px">
					<input type="checkbox" name="patientCheckbox" class="custom-control-input" id="icuCheckbox">
					<label for="icuCheckbox" class="custom-control-label normal-label">ส่งต่อไปรักษาที่</label>
					<input type="text" class="form-control form-control-sm ml-2" disabled style="width:200px">
				</div>
				<div class="custom-control custom-checkbox custom-control-inline">
					<input type="checkbox" name="patientCheckbox" class="custom-control-input" id="icuCheckbox">
					<label for="icuCheckbox" class="custom-control-label normal-label">เสียชีวิต</label>
				</div>
				<div class="custom-control custom-checkbox custom-control-inline">
					<input type="checkbox" name="patientCheckbox" class="custom-control-input" id="icuCheckbox">
					<label for="icuCheckbox" class="custom-control-label normal-label">ไม่ทราบ</label>
				</div>
			</div>
		</div>
	</div>
</div>
